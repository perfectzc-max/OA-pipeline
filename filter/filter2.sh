...
echo ${path}\/${cell}\.vcf  $currTime
#snv
grep '0/1' ${path}\/${cell}\.vcf | grep 'True' | awk '$7=="." && length($5)==1 && $6>=30'  | awk -F "[:,]" '$8+$9>=20' | grep -v "X" | grep -v "Y" | grep -v "MT" > ${cell}\.snv.vcf

#indel
grep '0/1' ${path}\/${cell}\.vcf | grep 'True' | awk '$7=="." && length($5)>1 && $6>=25' | awk -F "[:,]" '$8+$9>=30'| grep -v "X" | grep -v "Y" | grep -v "MT" > ${cell}\.indel.vcf
...
export NXF_SINGULARITY_CACHEDIR=/Software/nf_core/singularity_imgs/sarek_v2.7/
/Software/nf_core/NEXTFLOW/nextflow run /Software/nf_core/nf-core-sarek-2.7/workflow/  --input ${cell}\.indel.run.vcf --tools SnpEff  --step annotate  --outdir SnpEff_result  --genome GRCh37  --igenomes_base /Software/Refs/references  --species homo_sapiens  -profile singularity  -c /Software/nf_core/slurm_config/slurm_submit.config  -bg
